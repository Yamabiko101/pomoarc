...
fn render_header(frame: &mut Frame, state: &mut UiState, area: Rect) {
    let theme = state.theme();
    let titles = ["Timer", "Notes", "Stats", "Settings"]
        .into_iter()
...
fn render_notes(frame: &mut Frame, state: &UiState, area: Rect) {
    let theme = state.theme();
    let mut lines = vec![
        Line::styled(
            "Notes",
        ...
        Line::from(vec![
            ...
            Span::raw("pomoarc note add \"idea\""),
        ]),
        Line::from(vec![
            Span::styled("edit ", Style::default().fg(theme.warning)),
            Span::raw("pomoarc note edit 1 \"new text\""),
        ]),
        Line::from(vec![
            Span::styled("delete ", Style::default().fg(theme.danger)),
            Span::raw("pomoarc note delete 1"),
        ]),
        Line::raw(""),
    ];
